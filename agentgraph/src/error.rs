//! Error types for `agentgraph`.
//!
//! All fallible operations in the crate return [`Result<T>`] with
//! [`AgentGraphError`] as the error type.
...
impl AgentGraphError {
    ...
    pub fn is_interrupt(&self) -> bool {
        matches!(self, AgentGraphError::Interrupt { .. })
    }

    /// If this is an [`AgentGraphError::Interrupt`], returns a reference to
    /// the interrupt payload.
    pub fn interrupt_value(&self) -> Option<&Value> {
        match self {
            AgentGraphError::Interrupt { value } => Some(value),
            _ => None,
        }
    }
}
