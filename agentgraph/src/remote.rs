...
//!   [`AgentGraphError::Node`],
//! - `interrupt`— an interrupt payload, surfaced locally as
//!   [`AgentGraphError::Interrupt`] so HITL works across the wire
...
impl NodeTaskResponse {
    ...
    /// A failure response. Surfaced client-side as [`AgentGraphError::Node`].
    ...
    /// [`AgentGraphError::Interrupt`], so the executor's HITL machinery works
    ...
    /// - `error`    → `Err(AgentGraphError::Node(message))`
    /// - `interrupt`→ `Err(AgentGraphError::Interrupt { value })`
    ///
    ...
    pub fn into_result(self) -> Result<NodeOutput> {
        ...
        if let Some(message) = self.error {
            return Err(AgentGraphError::Node(message));
        }
...
impl RemoteNode {
    ...
    async fn try_once(
        ...
        response.json::<NodeTaskResponse>().await.map_err(|e| {
            AttemptError::Fatal(AgentGraphError::Node(format!(
