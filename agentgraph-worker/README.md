...
Worker-side SDK for [`agentgraph`](../agentgraph) remote node execution: *one
`Node` trait, remote impls behind the same trait*. A worker is an HTTP
...
and arrive client-side as `AgentGraphError::Node`, which the executor treats
