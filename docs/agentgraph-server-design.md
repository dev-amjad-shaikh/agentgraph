...
`agentgraph` today is a pure library: typed state channels with reducers, a Pregel/BSP
...
```
agentgraph/           # core engine — UNCHANGED. No HTTP, no axum, no server deps.
agentgraph-server/    # NEW (Phase A). axum + tokio. Depends on agentgraph.
                      #   Ships as a LIBRARY: users call agentgraph_server::serve().
agentgraph-proto/     # FUTURE (Phase B). protobuf/tonic contract for remote workers.
```

Dependency direction is strictly `user's binary → agentgraph-server → agentgraph`.
The core crate never learns that a server exists; `agentgraph-proto` is optional and only
...
│                                   │                                                │
│                    agentgraph_server::serve(registry, ServerConfig)                │
└───────────────────────────────────────┬────────────────────────────────────────────┘
                                        ▼
┌───────────────────────────── agentgraph-server ───────────────────────────────────┐
...
└───────────────┬───────────────────────────────────────┬────────────────────────────┘
                ...
                ▼                                     ▼
┌─────────────────────────────┐         ┌─────────────────────────────┐
│      agentgraph (core)      │         │  Checkpointer (trait)       │
...
async fn main() -> Result<()> {
    ...
    // --- one call: serve ------------------------------------------------- //
    let config = ServerConfig::from_env()?          // AGENTGRAPH_* env vars
...
agentgraph-server collapses this:
...
| Endpoint | agentgraph internal call |
|---|---|
| `GET /ok` | — (liveness) |
...
| `error` | `{error, message}` | `Err(AgentGraphError)` from `Executor::run` |
...
objection to out-of-process execution evaporates. And because `agentgraph`'s `State` is
...
long-poll named node-queues over gRPC (`agentgraph-proto`), receive
...
pub struct ServerConfig {
    pub bind_addr: SocketAddr,              // AGENTGRAPH_BIND_ADDR, default 0.0.0.0:8080
    pub checkpointer: Option<Arc<dyn Checkpointer>>,
    pub store_path: PathBuf,                // AGENTGRAPH_STORE_PATH (JsonFile dir)
    pub database_url: Option<String>,       // AGENTGRAPH_DATABASE_URL (Phase C, `postgres` feature)
    pub max_concurrent_runs_per_thread: usize, // default 1 (enqueue depth cap)
    pub api_keys: Vec<String>,              // AGENTGRAPH_API_KEYS (comma-separated; empty = dev mode, no auth)
...
**Phase A — the server crate (v0.2).** `agentgraph-server`: `GraphRegistry`, axum router
...
**Phase B — worker protocol.** `agentgraph-proto` (tonic): `PollNodeTask` /
...
**Phase A has shipped** as `agentgraph-server` v0.1.0 (library crate; `GraphRegistry`,
`ServerConfig`, `serve()` / `router()`), alongside core `agentgraph` v0.2.0 (Postgres
