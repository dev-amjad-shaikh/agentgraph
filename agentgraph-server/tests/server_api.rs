//! Integration tests for the agentgraph-server HTTP API, driven in-process
...
use agentgraph_server::{router, GraphRegistry, ServerConfig};
...
fn temp_store() -> PathBuf {
    std::env::temp_dir().join(format!("agentgraph-server-test-{}", uuid::Uuid::new_v4()))
}
...
async fn ok_and_info_list_graphs() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(v["service"], json!("agentgraph-server"));
