...
The trade is explicit: v0.1 is a **single-process** server. That covers the overwhelming majority of self-hosted agent deployments, and the `Node` trait keeps the multi-process door open — remote gRPC workers and WASM nodes are planned implementations of the same trait, not architectural changes.
...
An Agent-Protocol-compatible subset — wire-compatible with the core run/thread shapes LangGraph Platform uses, without the commercial surface. This table is the v0.1 endpoint inventory (Phase A); everything listed here is implemented and covered by integration tests.
...
Not in v0.1 (roadmap, see below): `GET /runs/{run_id}` run polling, thread listing/deletion endpoints, `/metrics`, `/graphs`. Thread records live in memory in v0.1 — checkpoints are durable on disk, but the thread registry itself is rebuilt empty on restart (re-create a thread with the same `thread_id` to re-attach to its on-disk checkpoints).
...
- [x] **Phase A — the server crate (v0.1).** `GraphRegistry`, the endpoint set above, per-thread run queue with `multitask_strategy` (`enqueue` / `reject`) plus explicit rollback via `DELETE /threads/{id}/runs/{run_id}`, SSE with mode filters (`updates` / `values` / `messages`) + per-run event log + `Last-Event-ID` dedup, static API-key middleware, `JsonFileCheckpointer` wiring from `ServerConfig::store_path`. *Shipped.*
...
- [ ] **Phase C — platform surface.** Durable thread registry, `GET /runs/{run_id}` polling, `WasmNode` (sandboxed wasmtime components behind the same trait — the only locality safe for untrusted/community nodes), **crons** (tokio scheduler + durable run queue), **assistants / config aliases**, cross-thread KV store, and wiring the core crate's `postgres` checkpointer feature into `ServerConfig`.

Deliberately skipped in v0.1: assistants/versioning (config *is* code; a "new version" is a recompile), crons (need a durable queue first), cross-thread store, A2A/MCP server endpoints, and WebSocket "protocol v2" (SSE + HTTP sidecar is sufficient).
